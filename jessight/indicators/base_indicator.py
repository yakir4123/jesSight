...
class BaseIndicator(CandlesProvider, abc.ABC):
    ...
    def _draw_marker(self, marker: Marker) -> None:
        ...
        self._chart_params[pconst.MARKER_CHART_PARAM].append(marker.to_dict())

    def _draw_line(self, value: LinePoint) -> None:
        timestamp = self.get_draw_timestamp()

        try:
            self._chart_params[pconst.LINE_CHART_PARAM][value.name]._values.append(value.value)
            self._chart_params[pconst.LINE_CHART_PARAM][value.name]._time.append(timestamp)
        except KeyError:
            raise KeyError(
    ...
    def get_draw_timestamp(self) -> int:
        return self.time - jh.timeframe_to_one_minutes(self.timeframe) * const.MILLISECONDS_IN_MINUTE
